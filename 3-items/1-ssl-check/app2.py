...
def check_ssl_expiration(domain: str, port: int = 443, verify_ssl: bool = True):
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python check_ssl_expiration.py <domain> [port] [--ignore-self-signed]")
        sys.exit(1)
    ...
    try:
        ...
        print(f"✅ SSL certificate for {domain} expires on {expiration_date} ({days_left} days left)")
    except Exception as e:
        print(f"❌ Error: {e}")
        sys.exit(1)
